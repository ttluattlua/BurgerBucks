...
START WITH 1
INCREMENT BY 1;
INSERT INTO BB_MEMBER VALUES(BB_MEMBER_SEQ.NEXTVAL,
'es153','es153','19891021','이서원','010-2648-2732',0,1,1,1,0)
...
FROM BB_ADDR WHERE DEL=0 ORDER BY SEQ DESC

SELECT SEQ, MEMBER_SEQ, ADDRESS, LAT, LNG, MEMO, DEL 
		FROM BB_ADDR WHERE DEL=0 AND MEMBER_SEQ=3
		ORDER BY SEQ DESC
...
UPDATE BB_ADDR 
TRUNCATE TABLE BB_ADDR;
CREATE SEQUENCE BB_ADDR_SEQ
START WITH 1
INCREMENT BY 1;

INSERT INTO BB_ADDR
VALUES(BB_ADDR_SEQ.NEXTVAL, 1,'동대문구 100-1','LAT','LNG','회사주소',0);
INSERT INTO BB_ADDR   
VALUES(BB_ADDR_SEQ.NEXTVAL, 3, '서울 강남구 남부순환로 2803 (도곡동, 삼성래미안아파트)', '37.4896766',  '127.05059189999997',   'ㅎㅎ', 0);
...
SELECT SEQ, MEMBER_SEQ, ADDRESS, LAT, LNG, MEMO, DEL 
		FROM BB_ADDR WHERE DEL=0 AND MEMBER_SEQ=1 
		ORDER BY SEQ DESC;
